...
public partial class PlatoonController : ControllerBase
{
	...
	[HttpGet("GetPlatoonsByCompany/{CompanyId:int}"), AllowAnonymous]
	public Task<IActionResult> GetPlatoonsByCompany(int CompanyId) =>
TryCatch(async () =>
{
	#region Validation
if (Convert.ToBoolean(_config["Hash:HashChecking"]))
...
var result = await _PlatoonRepository.GetPlatoonsByCompany(CompanyId);
if (result == null)
